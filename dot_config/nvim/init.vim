...
if !empty(glob(vim_plug_path))
    ...
    if has("gui_running")
        ...
        let g:colorscheme_manager_file = nvim_path . '/.gcolorscheme'
    endif
...
set number          " display absolute line number for current line
" }}}
" Searching {{{
set hlsearch
set incsearch
...
if has("gui_running")
    ...
    elseif has('gui_mac') || has('gui_macvim')
        set guifont=0xProtoNFM-Regular:h12,Monaco:h12
        if &guifont =~? 'NFM'
            set macligatures
        endif
    ...
    " Default mouse support on for all modes
    set mouse=a
endif
" }}}
